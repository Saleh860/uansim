...
public class Simulator {
	...
	public class Topology {
		public void setNodePositions(double[] nodePositions) {
			if (nodePositions.length % 3 != 0)
				System.err.println("nodePositions must be a nx3 vector");
			else {
				int n = nodePositions.length / 3;
				nodeCount = n - 1;
				this.nodePositions = new double[n][];
				for (int i = 0; i < n; i++) {
					this.nodePositions[i] = new double[] { nodePositions[i], nodePositions[i + n],
							nodePositions[i + 2 * n] };
				}
			}
		}
		...
		public void setAttackerPositions(double[] attackerPositions) {
			if (nodePositions.length % 3 != 0)
				System.err.println("attackerPositions must be a nx3 vector");
			else {
				int n = attackerPositions.length / 3;
				this.attackerPositions = new double[n][];
				for (int i = 0; i < n; i++) {
					this.attackerPositions[i] = new double[] { attackerPositions[i], attackerPositions[i + n],
							attackerPositions[i + 2 * n] };
				}
			}
		}
		...
		public void prepare() throws Exception {
			...
			// Choose the attacker node location to affect as many neighbors of the source
			// node as possible
		...
		public void createModel() throws Exception {
			...
			for (double commRange = 10; commRange < 1000; commRange += 10) {
				...
				double receivePower = topologyParameters.transmissionPower + gain
						- channel.getAmbientNoisePower(new double[] { 0, 0 });
			...
			// System.out.println("Calculated Communication Range=" +
			// topologyParameters.communicationRange);
		...
		public int countSourceNeighbors(int sourceId) throws Exception {
			...
			// Verify the network density by calculating the number of forwarders within the
			// communication
		...
		public void placeAttacker(int sourceNode) throws Exception {
			...
			double[] attackerPosition = new double[] { sourcePos[0], sourcePos[1], sourcePos[2] + 1.0 };

			placeAttacker(attackerPosition);

			// Verify that the attacker can reach all neighbors of source but reaches no
			// other nodes
	...
	/**
	 * The simulated topology parameters, specified before running the simulation
	 * and used for generating the simulated network.
	...
	public static class TopologyParameters {
		...
		/**
		 * The seed of the random number generator controlling the generation of nodes
		 * and other topology-related configuration
		 */
		public long topologySeed;
		/**
		 * The nominal communication range (at which the received signal to noise ratio
		 * is expected to be well above the detection threshold)
		 */
		public double communicationRange;
		/**
		 * The nominal interference range (at which the non-intended received signal
		 * would causes a noise powerful enough to
	...
	/**
	 * The simulated routing protocol parameters, specified before running the
	 * simulation and used for controlling routing decisions during simulation.
	 * 
	 * @author Saleh
	 *
	 */
	public static class RoutingParameters {
		/**
		 * The maximum number of hops before dropping the packet. Useful for avoiding
		 * routing loops.
		 */
	...
	public void makeAttackers() {
		attackers = new AttackNodeLayer[topology.attackerPositions.length];
		for (int i = 0; i < topology.attackerPositions.length; i++) {
			attackers[i] = network.new AttackNodeLayer(new Node(idGenerator, topology.attackerPositions[i]));
		}
	}
	...
	public void updateForwardingProbability() {
		for (int i = 0; i < topology.sourceNodes.length; i++) {
			routingParameters.forwardingProbability[topology.sourceNodes[i]] = routingParameters.K
	...
	public static void main(String[] args) throws IllegalArgumentException, IllegalStateException, InterruptedException,
		...
		try {
			sim.debug = true;
			sim.topology.sinkPos = new double[] { 250, 250, 0 };
