...
import uansim.RPR.AttackNodeLayer;
import uansim.DPR;
import uansim.RPR.Layer;
...
public class RPRSimulator {
	...
	public class Topology {
		...
		public void createModel() throws Exception {
			...
			// Create nodes
			makeSink(sinkPos);
			nodes = new Layer[nodePositions.length - 1];
	...
	public static class Results {
		...
		public int[] numberOfTransmissions; // per node
	...
	public RPR makeNetwork() {
		idGenerator = new SequentialIdGenerator();
		Channel channel = new Channel();
		physical = new Physical(channel);
		return network = new RPR(physical, new Random(routingParameters.decisionSeed));
	}

	public Layer makeSink(double[] pos) {
		sink = network.new Layer(new Node(idGenerator, pos));
		sink.setSink();
		return sink;
	}

	public Layer makeNode(double[] pos) {
		return network.new Layer(new Node(idGenerator, pos));
	}

	public Layer[] setSources(int[] sourceNodes) {

		sources = new Layer[sourceNodes.length];
		for (int i = 0; i < sourceNodes.length; i++)
			sources[i] = (Layer) network.getNodeById(sourceNodes[i]);
		return sources;
	}
	...
	public void run() throws Exception {
		...
		RPR.DISCOVERY_ROUNDS = routingParameters.discoveryRounds;
		RPR.MAX_TTL = routingParameters.maximumTTL;

		// TODO add RPR parameters
	...
	public static void main(String[] args) {
		RPRSimulator sim = new RPRSimulator();
		...
		sim.topologyParameters.attackersCount = 0;

		sim.trafficParameters.messageCount = 20;
		...
		try {
			...
			System.out.println("Testing " + RPRSimulator.class);
